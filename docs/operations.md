...
- **フォーク PR** — `claude-review.yml` は CI 成功時の `workflow_run` トリガーで動く
  ...
  なお**リリース PR (develop → main) は全量レビューをスキップする** — 含まれる各 PR が
  develop 向けに個別レビュー済みで全量レビューの必要性が薄く、巨大差分だと `error_max_turns` で
  ジョブが failure になり人間のマージ判断を萎縮させるため。gate ステップが `base == main` かつ
  head ブランチ `develop` を検出して決定論的に skip する (#80)
