...
- E2E でページ遷移 (フルロード) を挟む場合、遷移後の最初の要素待ちには長めの timeout (15s) を
  明示する。index.html の Google Fonts がブロッキング読み込みのため、外部ネットワークが遅い
  環境では React のマウントが既定の 5s を超えることがある (#63 で実測)
